...
SELECT
    ...
    (SELECT SUM(COALESCE(TRY_CAST(v AS DOUBLE), 0)) FROM (VALUES
        (LEASE_GAS_DISPCD01_VOL), (LEASE_GAS_DISPCD02_VOL), (LEASE_GAS_DISPCD03_VOL),
        (LEASE_GAS_DISPCD04_VOL), (LEASE_GAS_DISPCD05_VOL), (LEASE_GAS_DISPCD06_VOL),
        (LEASE_GAS_DISPCD07_VOL), (LEASE_GAS_DISPCD08_VOL), (LEASE_GAS_DISPCD09_VOL),
        (LEASE_GAS_DISPCD99_VOL)
    ) t(v)) AS lease_gas_total_vol,
    (SELECT SUM(COALESCE(TRY_CAST(v AS DOUBLE), 0)) FROM (VALUES
        (LEASE_CSGD_DISPCDE01_VOL), (LEASE_CSGD_DISPCDE02_VOL), (LEASE_CSGD_DISPCDE03_VOL),
        (LEASE_CSGD_DISPCDE04_VOL), (LEASE_CSGD_DISPCDE05_VOL), (LEASE_CSGD_DISPCDE06_VOL),
        (LEASE_CSGD_DISPCDE07_VOL), (LEASE_CSGD_DISPCDE08_VOL), (LEASE_CSGD_DISPCDE99_VOL)
    ) t(v)) AS lease_csgd_total_vol,
