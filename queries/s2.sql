...
-- quality filtering of its own — no score cut, no ranking, and (since gaslight
-- now covers the New Mexico Delaware side too, via the NM OCD layer) no
-- geographic clip either: we ingest every site p-f publishes and only re-round
-- the columns for a tidy committed file. Each site carries its per-date
-- observations as a `detections` JSON array (date, max_b12, pixels), preserving
-- the timeline.
--
