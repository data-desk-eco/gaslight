...
- **Lease footprints**: `rrc.leases` — union of OTLS survey polygons per lease. Leases spanning >10km extent excluded as data errors. Exported as `lease_footprints.parquet` with simplified geometry (ST_AsGeoJSON) and aggregated flaring volumes from `rrc.production`. Only leases with reported flaring are exported. Rendered client-side as polygon layer colored by flaring intensity (flared/produced %).
