...
- **drawer.js** — data drawer with tabbed tables (flares/permits/plumes/wells/leases), column sorting, keyboard navigation (j/k/h/l/g/G), viewport-synced queries.
...
6. **Reported flaring volumes**: Monthly lease-level gas disposition data from RRC PDQ (Production Data Query). Disposition code 04 = gas vented/flared. `rrc.production` stores monthly totals per lease (gas flared MCF + casinghead gas flared MCF). Flaring intensity = flared gas / total gas produced (%). Aggregated to lease footprints for map layer, and shown per-lease in flare detail panels.
...
- **Lease footprints**: `rrc.leases` — union of OTLS survey polygons per lease. Leases spanning >10km extent excluded as data errors. Exported as `lease_footprints.parquet`, dissolved by `(district, abstract_set)` so vertically stacked leases sharing the same OTLS surveys become a single polygon (~47K → ~8K polygons). Each footprint carries a `leases` JSON array with per-lease details (operator, name, wells, flared/produced MCF). Only leases with reported flaring are exported. Rendered client-side as polygon layer colored by flaring intensity. Clicking a footprint shows individual lease cards with monthly production charts.
...
- **Colors**: defined centrally as CSS custom properties (`--color-flare`, `--color-permit`, `--color-plume`, `--color-well`, `--color-lease`) in `:root`; JS reads them via `getComputedStyle`. Color ramps for intensity (`b12Color`, `mwColor`) are shared functions in app.js. Lease footprints use a green→red fill ramp based on flaring intensity %.
