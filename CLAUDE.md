...
- `web/s2.js` — loads the S2 site catalogue (`web/data/s2.parquet`, site-level metadata only) for display; per-date detections live in `web/data/s2_detections.parquet`, fetched lazily by h3 on card open
...
- **s2.js** — loads permian-flaring's score-capped S2 site catalogue (`s2.parquet`, metadata only) into a `Map<h3, site>`. Per-date detections are split into `s2_detections.parquet` and fetched lazily by h3 (`db.queryS2Detections`) when a card opens, then drive the event list + timeline chart. Display-only; no detection runs in-browser.
...
- **Wells (two states)**: the "Oil/gas wells" map layer and the shareable DB carry both states as a symmetric pair, both rendered as plain white X markers (SDF symbols). Texas wells (`wells_tx` table → `web/data/wells_tx.parquet`, `wells-tx-layer`) come from the RRC and still carry per-lease flaring metrics (`flared_mcf`, `produced_mcf`, `flaring_intensity_pct`) joined from PDQ production data — shown in the detail card's lease/flaring/production section, no longer used to colour the marker (the intensity ramp added little once the layer was restricted to a flare radius). New Mexico wells (`wells_nm` table → `web/data/wells_nm.parquet`, `wells-nm-layer`) come from the NM OCD well search (active, un-plugged, surface locations); no PDQ flaring feed exists on the NM side, so they carry header attributes only (type, status, spud, depths, operator). Both ride the single "Wells" toggle; each gets its own drawer tab ("TX wells" / "NM wells") and detail card. The web layer ships only wells within the VIIRS pixel bbox (±0.0034° ≈ 375m — the "flare detection area" box) of a flare site (VNF or S2 — some S2 sites have no VNF counterpart); the full whole-Permian sets live in `dist.wells_tx` / `dist.wells_nm`.
...
- **IMEO source**: `data/imeo_plumes.geojson` — manual download from methanedata.unep.org (no API). Sentinel-5P/TROPOMI plumes are kept in the DB but excluded from the web export (too coarse, ~7km pixels).
...
- **Colors**: defined centrally as CSS custom properties (`--color-flare`, `--color-permit`, `--color-plume`, `--color-well`) in `:root`; JS reads them via `getComputedStyle`. Color ramps for intensity (`b12Color`, `mwColor`) are shared functions in app.js. Wells (both states) render as plain white X markers — no intensity ramp.
